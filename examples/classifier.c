...
void train_classifier_pretrained(char *solver, char *cfgfile, char *cfgfile_valid, char *weightfile, char *cfgfile_pretrained, int *gpus, int ngpus, int clear)
{
    int i;
    ...
    while(get_current_batch(net) < net->max_batches || net->max_batches == 0){
        ...
        if(get_current_batch(net)%test_interval == 0){  // Test_interval
            ...
            //validate_classifier_single(solver, cfgfile_valid, buff);
...
void train_classifier(char *solver, char *cfgfile, char *cfgfile_valid, char *weightfile, int *gpus, int ngpus, int clear)
{
    int i;
...
// Get outputs from a pretrained network
void get_logits(char *solver, char *cfgfile, char *weightfile)
{
    int curr = 0;
    network *net = load_network(cfgfile, weightfile, 0);
    srand(time(0));
    //
    fprintf(stderr,"\nnet.outputs = %d",net->outputs);
    layer out = get_network_output_layer(net);
    fprintf(stderr,"\nout.out_h = %d",out.out_h);
    fprintf(stderr,"\nout.out_w = %d",out.out_w);
    fprintf(stderr,"\nout.out_c = %d",out.out_c);
    //

    list *options = read_data_cfg(solver);
    char *test_list = option_find_str(options, "train", "data/train.list");
    char *target_folder = option_find_str(options, "target_folder", "targets");
    ...
    for(curr = net->batch; curr <= m; curr += net->batch){
        ...
        for(i = 0; i < pred.rows; ++i){
            char labelpath[4096];
            find_replace(paths[curr-net->batch+i], "images", target_folder, labelpath);
            find_replace(labelpath, ".jpg", ".txt", labelpath);
            find_replace(labelpath, ".JPEG", ".txt", labelpath);
            find_replace(labelpath, ".png", ".txt", labelpath);
            FILE *file = fopen(labelpath, "w");
            for(j = 0; j < pred.cols; ++j){
                fprintf(file,"%.4f ", pred.vals[i][j]);
            }
            fclose(file);
        }
        ...
        if(curr%100 == 0) { fprintf(stderr, "\n%d", curr); }
    }
}


// Get outputs for object regions
void get_regions(char *solver, char *cfgfile, char *weightfile)
{
    int curr = 0;
    network *net = load_network(cfgfile, weightfile, 0);
    srand(time(0));
    ...
    for(curr = net->batch; curr <= m; curr += net->batch){
        ...
        time=clock();
        matrix pred = network_predict_data_truth(net, val);
...
void run_classifier(int argc, char **argv)
{
    ...
    else     // Valid/Predict/Test arguments
    {
        ...
        filename = argv[6]; // test image name
    ...
    if(0==strcmp(argv[2], "train")){
        if(argc <=6)
            train_classifier(solver, cfg, cfg_valid, weights, gpus, ngpus, clear);
        else
            train_classifier_pretrained(solver, cfg, cfg_valid, weights, cfg_pretrained, gpus, ngpus, clear);
    }
    else if(0==strcmp(argv[2], "valid")){ 
        //validate_classifier_single(solver, cfg, weights);
    }
    else if(0==strcmp(argv[2], "predict")){
         predict_classifier(solver, cfg, weights, filename, top);
    }
    else if(0==strcmp(argv[2], "get_logits")){
         get_logits(solver, cfg, weights);
    }
    else if(0==strcmp(argv[2], "get_regions")){
         get_regions(solver, cfg, weights);
    }
    else if(0==strcmp(argv[2], "test")){
         test_classifier(solver, cfg, weights, layer);
    }    
    else if(0==strcmp(argv[2], "fout")) file_output_classifier(solver, cfg, weights, filename);
    else if(0==strcmp(argv[2], "try")) try_classifier(solver, cfg, weights, filename, atoi(layer_s));
    else if(0==strcmp(argv[2], "demo")) demo_classifier(solver, cfg, weights, cam_index, filename);
    else if(0==strcmp(argv[2], "gun")) gun_classifier(solver, cfg, weights, cam_index, filename);
    else if(0==strcmp(argv[2], "threat")) threat_classifier(solver, cfg, weights, cam_index, filename);
    else if(0==strcmp(argv[2], "label")) label_classifier(solver, cfg, weights);
    else if(0==strcmp(argv[2], "validmulti")) validate_classifier_multi(solver, cfg, weights);
    else if(0==strcmp(argv[2], "valid10")) validate_classifier_10(solver, cfg, weights);
    else if(0==strcmp(argv[2], "validcrop")) validate_classifier_crop(solver, cfg, weights);
    else if(0==strcmp(argv[2], "validfull")) validate_classifier_full(solver, cfg, weights);
}
