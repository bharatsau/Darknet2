...
//
__device__ float negative_relu_activate_kernel(float x){return x*(x<0);}
__device__ float abs_activate_kernel(float x){return (x>0) ? x : -1.0*x;}
__device__ float square_activate_kernel(float x){return x*x;}
//
...
//
__device__ float negative_relu_gradient_kernel(float x){return (x<0);}
__device__ float abs_gradient_kernel(float x){return (x>0) ? 1 : -1;}
__device__ float square_gradient_kernel(float x){return 2*sqrt(x);}
//
...
__device__ float activate_kernel(float x, ACTIVATION a)
{
    switch(a){
        //
        case ABS:
            return abs_activate_kernel(x);
        case SQUARE:
            return square_activate_kernel(x);
        case NEGATIVE_RELU:
            return negative_relu_activate_kernel(x);        
        //
...
__device__ float gradient_kernel(float x, ACTIVATION a)
{
    switch(a){
        //
        case ABS:
            return abs_gradient_kernel(x);
        case SQUARE:
            return square_gradient_kernel(x);
        case NEGATIVE_RELU:
            return negative_relu_gradient_kernel(x);        
        //
