...
__global__ void eltwise_kernel(int size, int minw, int minh, int minc, int stride, int sample, int batch, int w1, int h1, int c1, float *add, int w2, int h2, int c2, float *out, int sum, int mult)
{
    int id = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= size) return;
    int i = id % minw;
    ...
    if(mult==1)
        out[out_index] = out[out_index] * add[add_index];  // Eltwise multiplication
    else if(sum==1)
        out[out_index] = out[out_index] + add[add_index];  // Eltwise summation
}

extern "C" void eltwise_gpu(int batch, int w1, int h1, int c1, float *add, int w2, int h2, int c2, float *out, int sum, int mult)
{
    int minw = (w1 < w2) ? w1 : w2;
    int minh = (h1 < h2) ? h1 : h2;
    int minc = (c1 < c2) ? c1 : c2;
    ...
    int size = batch * minw * minh * minc;
    eltwise_kernel<<<cuda_gridsize(size), BLOCK>>>(size, minw, minh, minc, stride, sample, batch, w1, h1, c1, add, w2, h2, c2, out, sum, mult);
    check_error(cudaPeekAtLastError());
}
