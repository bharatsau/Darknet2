...
cost_layer make_cost_layer(int batch, int inputs, COST_TYPE cost_type, float scale, int multiloss)
{
    ...
    l.multiloss = multiloss;
    
    l.forward = forward_cost_layer;
    l.backward = backward_cost_layer;
    if(l.multiloss)
        l.forward = forward_cost_layer_regression;

    #ifdef GPU
    l.forward_gpu = forward_cost_layer_gpu;
    l.backward_gpu = backward_cost_layer_gpu;
    if(l.multiloss)
        l.forward_gpu = forward_cost_layer_regression_gpu;
...
// For multiloss regression and detection
void forward_cost_layer_regression(cost_layer l, network net)
{
    if (!l.multiloss) return;

    if(l.cost_type == SMOOTH){
        smooth_l1_cpu(l.batch*l.inputs, net.input, net.truth_regression, l.delta, l.output);
    }else if(l.cost_type == L1){
        l1_cpu(l.batch*l.inputs, net.input, net.truth_regression, l.delta, l.output);
    } else {
        l2_cpu(l.batch*l.inputs, net.input, net.truth_regression, l.delta, l.output);
    }
    l.cost[0] = sum_array(l.output, l.batch*l.inputs);
}
...
// For multiloss regression and detection
void forward_cost_layer_regression_gpu(cost_layer l, network net)
{
    if (!l.multiloss) return;
    if(l.smooth){
        scal_gpu(l.batch*l.inputs, (1-l.smooth), net.truth_regression_gpu, 1);
        add_gpu(l.batch*l.inputs, l.smooth * 1./l.inputs, net.truth_regression_gpu, 1);
    }

    if(l.cost_type == SMOOTH){
        smooth_l1_gpu(l.batch*l.inputs, net.input_gpu, net.truth_regression_gpu, l.delta_gpu, l.output_gpu);
    } else if (l.cost_type == L1){
        l1_gpu(l.batch*l.inputs, net.input_gpu, net.truth_regression_gpu, l.delta_gpu, l.output_gpu);
    } else if (l.cost_type == WGAN){
        wgan_gpu(l.batch*l.inputs, net.input_gpu, net.truth_regression_gpu, l.delta_gpu, l.output_gpu);
    } else {
        l2_gpu(l.batch*l.inputs, net.input_gpu, net.truth_regression_gpu, l.delta_gpu, l.output_gpu);
    }

    if (l.cost_type == SEG && l.noobject_scale != 1) {
        scale_mask_gpu(l.batch*l.inputs, l.delta_gpu, 0, net.truth_regression_gpu, l.noobject_scale);
        scale_mask_gpu(l.batch*l.inputs, l.output_gpu, 0, net.truth_regression_gpu, l.noobject_scale);
    }
