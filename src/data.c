...
// Modified for Knowledge Distillation/Dark Knowledge
matrix load_regression_labels_paths(char **paths, int n)
{
    matrix y_regression = make_matrix(n, target_length);
    
    int i,j;
    for(i = 0; i < n; ++i){
        char labelpath[4096];
        // Path to file
        find_replace(paths[i], "images", target_folder, labelpath);
        find_replace(labelpath, "JPEGImages", target_folder, labelpath);
        // .txt for label file
        ...
        for(j = 0; j < target_length; j++){
            fscanf(file, "%f ", &(y_regression.vals[i][j]));    
        }
        fclose(file);        

        // If softmax_Temperature>1        
        if(softmax_Temperature>1){  softmax_cpu(y_regression.vals[i], target_length, 1, target_length, 1, target_length, 1, (float)softmax_Temperature, y_regression.vals[i]);  }
...
// Dataload for object_regions predict case
data load_data_old_detection(int batch, char **paths, int w, int h, int boxes)
{
    int i;
    data d = {0};
    d.shallow = 0;
    ...
    d.y = make_matrix(batch, 5*boxes);
    for(i = 0; i < batch; ++i){
        image orig = load_image_color(paths[i], 0, 0);
        image sized = make_image(w, h, orig.c);
        fill_image(sized, .5);
        place_image(orig, w, h, 0, 0, sized);
        d.X.vals[i] = sized.data;

        fill_truth_detection(paths[i], boxes, d.y.vals[i], 0, 0, 0, 0, 1, 1);
...
// Dataload for object_regions training case
data load_data_region_regression_det(int n, char **paths, int m, int w, int h, int boxes, int classes)
{
    int i;
    data d = {0};
    d.shallow = 0;
    ...
    for(i = 0; i < n; ++i){
        ...
        fill_truth_detection(random_paths[i], boxes, d.y.vals[i], classes, 0, 0, 0, 1, 1);

        free_image(orig);
    }
    // Load Regression data
    d.y_regression = load_regression_labels_paths(random_paths, n);
    
    free(random_paths);    
    return d;
}

// For loading both detection and regression data
data load_data_detection_regression(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    char **random_paths = get_random_paths(paths, n, m);
    int i,j;
    data d = {0};
...
void *load_thread(void *ptr)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
        ...
        *a.d = load_data_regression(a.paths, a.n, a.m, a.w, a.h);
    ...
    } else if (a.type == OLD_DETECTION_DATA){
        *a.d = load_data_old_detection(a.n, a.paths, a.w, a.h, a.num_boxes);
    } else if (a.type == REGION_REGRESSION_DET_DATA){
        *a.d = load_data_region_regression_det(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes);
    } else if (a.type == DETECTION_REGRESSION_DATA){
        *a.d = load_data_detection_regression(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
...
// Data regression input
data load_data_regression(char **paths, int n, int m, int w, int h)
{
    if(m) paths = get_random_paths(paths, n, m);
    data d = {0};
    d.shallow = 0;
...
data concat_data(data d1, data d2)
{
    ...
    // For regression with classification or detection
    if(multiloss_cost){
        d.y_regression = concat_matrix(d1.y_regression, d2.y_regression); 
    }
    d.w = d1.w;
    d.h = d1.h;
    return d;
...
// For multiloss regession with classification or detection
void get_next_batch_multiloss_regression(data d, int n, int offset, float *X, float *y, float *y_regression)
{
    int j;
    for(j = 0; j < n; ++j){
        int index = offset + j;
        ...
        if(y) memcpy(y+j*d.y.cols, d.y.vals[index], d.y.cols*sizeof(float));
        if(multiloss_cost){
            memcpy(y_regression+j*d.y_regression.cols, d.y_regression.vals[index], d.y_regression.cols*sizeof(float));         
        }
    }
}
